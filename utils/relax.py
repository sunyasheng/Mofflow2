...
def initiate_lammps_with_force_field(cif_file, force_field="UFF"):
    ...
    logname = Path(cif_file).parent.parent / "relaxed" / f"{fname}" / "log.lammps"
    logname.parent.mkdir(exist_ok=True, parents=True)
    lmp = lammps(cmdargs=["-log", str(logname)])
...
def dump_to_structure(dump_file, data_file):
    ...
    element_symbols = np.array(
        [element.symbol for element in mendeleev.get_all_elements()]
    )
    element_mass = np.array(
        [element.mass for element in mendeleev.get_all_elements()]
    )
