...
def process_one(idx, value):
    ...
    try:
        ...
        # Extract torsion scores
        bb_mols = [Chem.MolFromMolBlock(mol_str, removeHs=False) for mol_str in feats['bb_mols']]

        total_symmetry_scores = []
        for i, bb_mol in enumerate(bb_mols):
            bb_feats = mu.featurize_mol(bb_mol)
            bond_mask, atom_mask = mu.get_transformation_mask(bb_feats, bb_mol)

            symmetry_scores = tu.get_torsion_symmetry_scores(
                bb_mol=bb_mol,
                bond_index=bb_feats.edge_index,
                rotable_bond_mask=bond_mask,
                rotable_atom_mask=atom_mask,
            )
            total_symmetry_scores.append(symmetry_scores)
        total_symmetry_scores = torch.cat(total_symmetry_scores, dim=0)  # [total_num_rotatable_bonds]
        feats['symmetry_scores'] = total_symmetry_scores
...
class ExtractFeatures:
    ...
    def process(self, split="train"):
        ...
        # Read data
        data_dict = {}
        src_env = read_lmdb(f"{self.lmdb_dir}/MetalOxo_mofchecker_matched_{split}.lmdb")
        ...
        dest_env = write_lmdb(f"{self.lmdb_dir}/MetalOxo_final_{split}.lmdb")
